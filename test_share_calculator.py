...
from shares_calculator_application_main import is_father_alive,calculate_share_and_deduct_from_total,calculate_shares_for_spouse_with_sons_or_daughters
...
class TestFamilyStatus(unittest.TestCase):
    ...
    @patch('SharesApplicationMain.calculate_share_and_deduct_from_total')
    def test_distribute_amount(self, mock_calculate_and_deduct):
        mock_calculate_and_deduct.return_value = (250.0, 750.0)
        part, remaining = calculate_share_and_deduct_from_total(1000, 0.25, 1)
...
class TestFamilyStatus(unittest.TestCase):
    ...
    def test_calculate_and_deduct(self):
        ...
        self.assertEqual(part, 250.0)
        self.assertEqual(remaining, 750.0)
        part, remaining = calculate_share_and_deduct_from_total(500, 0.5, 2)
        self.assertEqual(part, 250.0)
